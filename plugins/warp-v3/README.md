...
|---|---|
| Fresh tabs emit **nothing** at all — parity with Gemini CLI + Factory/Droid | Warp's default state for a Claude Code session_start is "running" / In progress. Forcing it to "Done" via a synthetic stop (v3.0.4) was worse than the no-emission alternative we observed for Gemini + Droid: their sidebar rows show **just the CLI label + cwd + branch, no state pill at all**, because neither CLI registers a Warp hook. We now match that — `on-session-start.sh` exits silently for `source=startup`. Warp's process-detection still registers the sidebar row; first state transition fires only on `prompt_submit` (→ running) → `stop` (→ done). For resume/clear/compact, we still emit `session_start` with enrichment since those genuinely change state. `plugin_version` is attached to the first `prompt_submit` instead of `session_start` so Warp's outdated-plugin banner still has a signal. |
